...
def load_bb_data(
    data_dir: str, param_combo: str, batch_name: str, max_samples: int = 10000
):
    """
    ...
    if max_samples < committed_clusters_csr.shape[0]:
        print(
            f"Limiting to first {max_samples} samples (out of {committed_clusters_csr.shape[0]})"
        )
...
def run_benchmark():
    """
    Run benchmarking on the optimized igraph implementation.
    """
    print("=" * 80)
    print("BENCHMARKING: Optimized igraph implementation")
    print("=" * 80)
    ...
    try:
        ...
        for i, config in enumerate(test_configs, 1):
            ...
            result = calculate_committed_cluster_norm_fractions_from_csr(
                ...
                _benchmarking=True,  # Enable detailed benchmarking
            )

            elapsed_time = time.perf_counter() - start_time

            print(f"\nTotal execution time: {elapsed_time:.4f}s")
