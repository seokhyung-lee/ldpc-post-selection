...
################################################################################
# Sliding window decoding metrics
################################################################################
    ...
    for sample_idx in range(num_samples):
        ...
        if value_type == "size":
            ...
            unique_clusters, cluster_sizes = np.unique(
                committed_cluster_labels[committed_cluster_labels > 0],
                return_counts=True,
            )
            ...
            if len(cluster_sizes) > 0:
                ...
                norm_fractions[sample_idx] = (
                    inside_norm / total_faults if total_faults > 0 else 0.0
                )
            else:
                norm_fractions[sample_idx] = 0.0
            ...
            if len(unique_clusters) > 0:
                ...
                cluster_llr_sums = np.bincount(
                    inverse_indices,
                    weights=bit_llrs[
                        committed_fault_indices[committed_cluster_labels > 0]
                    ],
                )
