...
import \
    itt  # install see https://github.com/oleksandr-pavlyk/itt-python, you also need to install VTune
...
def sliding_window_decoder(N,
                           ...
                           last_win_gdg_factor=1.0,
                           last_win_bp_factor=1.0):
    if N == 72:
        code, A_list, B_list = create_bivariate_bicycle_codes(6,
                                                              6,
                                                              [3],
                                                              [1, 2],
                                                              [1, 2],
        ...
        code, A_list, B_list = create_bivariate_bicycle_codes(15,
                                                              3,
                                                              [9],
                                                              [1, 2],
                                                              [2, 7],
        ...
        code, A_list, B_list = create_bivariate_bicycle_codes(9,
                                                              6,
                                                              [3],
                                                              [1, 2],
                                                              [1, 2],
        ...
        code, A_list, B_list = create_bivariate_bicycle_codes(12,
                                                              6,
                                                              [3],
                                                              [1, 2],
                                                              [1, 2],
        ...
        code, A_list, B_list = create_bivariate_bicycle_codes(30,
                                                              6,
                                                              [9],
                                                              [1, 2],
                                                              [25, 26],
        ...
        code, A_list, B_list = create_bivariate_bicycle_codes(21,
                                                              18,
                                                              [3],
                                                              [10, 17],
                                                              [3, 19],
    ...
    chk, obs, priors, col_dict = dem_to_check_matrices(dem,
    ...
    chk = np.concatenate([chk[:, col].toarray() for col in region_cols],
                         axis=1)
    obs = np.concatenate([obs[:, col].toarray() for col in region_cols],
                         axis=1)
    ...
    if noisy_prior is None and method != 0:
        ...
        noisy_prior = np.sum(chk[c[0]:b[0], c[1]:b[1]] * priors[c[1]:b[1]],
                             axis=1)
    ...
    for i in range(num_win):
        ...
        if i != num_win - 1 and method != 0:  # not the last round
            ...
            noisy_syndrome[-n_half:,
            :] = np.eye(n_half)  # * noisy_syndrome_prior
    ...
    det_data, obs_data, err_data = dem_sampler.sample(shots=num_shots,
                                                      return_errors=False,
                                                      bit_packed=False)
    end_time = time.perf_counter()
    print(f"Stim: noise sampling for {num_shots} shots, elapsed time:",
          end_time - start_time)
    ...
    while i < num_win:
        ...
        if i == num_win - 1 and osd:
            bpd = bposd_decoder(
                ...
                error_rate=p,
                # does not matter because channel_probs is assigned
                channel_probs=prior,
            ...
            bpgdg = bpgdg_decoder(
                ...
                ms_scaling_factor=last_win_bp_factor if (
                            i == num_win - 1) else 1.0,
            )
        num_flag_err = 0
