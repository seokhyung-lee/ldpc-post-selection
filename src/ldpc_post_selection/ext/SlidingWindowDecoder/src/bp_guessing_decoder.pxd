...
cdef extern from "bpgd.hpp":
    ...
    cdef cppclass BPGD:
        ...
        int reset(mod2sparse * source_pcm,
                  int * copy_cols,
                  double * source_llr_prior,
                  char * source_syndrome)
        void set_masks(char * source_vn_mask,
                       char * source_cn_mask,
                       char * source_cn_degree)
        double get_pm()
        BPGD(int m,
             int n,
             int num_iter,
             int low_error_mode,
             double factor) except +
        BPGD()

    cdef cppclass BPGD_main_thread(BPGD):
        char * min_pm_error
        double min_pm
        void do_work(mod2sparse * source_pcm,
                     int * copy_cols,
                     double * source_llr_prior,
                     char * source_syndrome);
        BPGD_main_thread(int m, int n, int low_error_mode) except +
        BPGD_main_thread(int m,
                         int n,
                         int num_iter,
                         int max_step,
                         int max_tree_depth,
