...
def data_qubit_noise_decoding(code,
                              p,
                              num_shots=1000,
                              osd_orders=[10],
                              osd_factor=0.625,
    ...
    for order in osd_orders:
        ...
        bpd = bposd_decoder(
            ...
            error_rate=p,
            # does not matter as channel_prob is set
            channel_probs=priors,
            # override "error_rate" input variable
            max_iter=100,
            bp_method="minimum_sum_log",
            # messages are not clipped, may have numerical issues
            ms_scaling_factor=osd_factor,
            # usually {0.5, 0.625, 0.8, 1.0} suffice
            osd_method="osd_cs",
    ...
    bpgdg = bpgdg_decoder(
        ...
        gdg_factor=0.625,
        max_step=max_step,
        # have to use larger max_step for longer block-length codes
        max_tree_depth=4,
        max_side_depth=20,
