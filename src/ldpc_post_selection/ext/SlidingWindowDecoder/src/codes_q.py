...
from .utils import compute_code_distance, int2bin, inverse, kernel, rank, \
    row_echelon
...
class css_code():  # a refactored version of Roffe's package
    ...
    def __init__(self,
        ...
        self.hx_basis = self.hx[
            self.pivot_hx]  # same as calling row_basis(self.hx)
        self.hz_basis = self.hz[
            self.pivot_hz]  # but saves one row echelon calculation
        self.K = self.N - self.rank_hx - self.rank_hz

        self.compute_ldpc_params()
        self.compute_logicals()
        if code_distance is np.nan:
            dx = compute_code_distance(self.hx_perp,
                                       is_pcm=False,
                                       is_basis=True)
            dz = compute_code_distance(self.hz_perp,
                                       is_pcm=False,
                                       is_basis=True)
            self.D = np.min([dx,
                             dz])  # this is the distance of stabilizers, not the distance of the code
    ...
    def compute_logicals(self):
        ...
        def compute_lz(ker_hx, im_hzT):
            ...
            log_op_indices = [i for i in
                              range(im_hzT.shape[0], log_stack.shape[0]) if
                              i in pivots]
...
def create_bivariate_bicycle_codes(l,
                                   m,
                                   A_x_pows,
                                   A_y_pows,
                                   B_x_pows,
...
def construct_vector(m, log_table, antilog_table):
    ...
    for i in range(1,
                   gf_size):  # Skip 0 as alpha^0 = 1, and we are interested in alpha^i where i != 0
        # Calculate 1 + alpha^i in GF(2^m)
        # Since addition is XOR in GF(2^m), and alpha^0 = 1, we use log/antilog tables
        sum_val = 1 ^ antilog_table[i % (
                    gf_size - 1)]  # Note: antilog_table[log_val % (gf_size - 1)] == alpha^i
...
def create_EG_codes(s):
    ...
    log_table, antilog_table = generate_log_antilog_tables(extension,
